...
def socrat(n,m):
    ...
    return n//k, m//k
def line_spec(A, B, C):
    # Совпадает с одной из координат, или просто проходит через начало координат
    if not C:
        if not A:
            I = "Прямая совпадает с осью Абсцисс (Ox)\n(A и C = 0)"
        elif not B:
            I = "Прямая совпадает с осью Ординат (Oy)\n(B и C = 0)"
        else:
            I = "Прямая проходит через начало координат\n(C = 0)"
    elif not A:
        I = "Прямая параллельна оси Абсцисс (Ox)\n(A = 0)"
    elif not B:
        I = "Прямая параллельна оси Ординат (Oy)\n(B = 0)"
    else:
        I = "Прямая не параллельна, и не совпадает ни с одной осью, а также не проходит через начало координат"
    return I
...
# Прямые
def choice7():
    print("\nЭтот блок вычисляет координаты точки пересечения двух прямых\nv.0.1.3")
    while True:
        # ввод
        line1 = input("В уравнение вида Ax+By+C=0 описывающую ПЕРВУЮ прямую введите значения коэфициентов A, B, C подряд через пробелы\nA²+B²≠0\nНапример: 3 -4 -6\n")
        line1 = line1.split()
        # проверка 1
        if len(line1) != 3:
            print("Неверное количество символов!")
        ...
        # ввод
        line2 = input("В уравнение вида Ax+By+C=0 описывающую ВТОРУЮ пямую введите значения коэфициентов A, B, C подряд через пробелы\nA²+B²≠0\nНапример: 2 5 2\n")
        line2 = line2.split()
        # проверка 2
        if len(line1) != 3:
            print("Неверное количество символов!")
        ...
        if h == D2: 
            ...
            print("y = {}/{} = {}/{} = {}".format(D2,D,h,g,k))

        # Проверка -----------------------------
        print("\nПроверка")
        print("{"+"{}*({}/{}){}*({}/{}) = {}".format(line1[0], i, j, line1_out[1], h, g, line1_out[2]))
        print("{"+"{}*({}/{}){}*({}/{}) = {}\n".format(line2[0], i, j, line2_out[1], h, g, line2_out[2]))

        print("{"+"{} = {}".format(line1_out[2], line1_out[2]))
        print("{"+"{} = {}\n".format(line2_out[2], line2_out[2]))
        return
def choice8_1():
    print("\nЭтот блок переводит из одно уравнение прямой в другое\nv.0.1.0")
    while True:
        # ввод
        line = input("В уравнение вида Ax+By+C=0 описывающую прямую введите значения коэфициентов A, B, C подряд через пробелы\nB≠0\nНапример: 3 -4 -6\n")
        line = line.split()
        # проверка 1
        prov = ''
        for i in line:
        ...
        # проверка 3
        if not B:
            print("Сожалеем, но у вас B = 0\nА по правилам B не должно ровняться 0, т.к. в таком случае y = 0\nПогугли что это значит..")
            break
        # загутая страница в тетради по математике
        line_int = []
        for i in line:
            line_int.append(int(i))

        k = -(line_int[0]/line_int[1])
        b = -(line_int[2]/line_int[1])

        k_1, k_2 = socrat(line_int[0], line_int[1])
        b_1, b_2 = socrat(line_int[2], line_int[1])
        
        if k_1 == line_int[0]:
            if b_1 == line_int[2]:
                print("y = -({}/{})x-({}/{})".format(line_int[0], line_int[1], line_int[2], line_int[1]))
            else:
                print("y = -({}/{})x-({}/{})".format(line_int[0], line_int[1], b_1, b_2))
        else:
            if b_1 == line_int[2]:
                print("y = -({}/{})x-({}/{})".format(k_1, k_2, line_int[2], line_int[1]))
            else:
                print("y = -({}/{})x-({}/{})".format(k_1, k_2, b_1, b_2))
        if b>=0:
            b = ("("+str(b)+")")
...
def choice8_2():
    pass
def choice9():
    print("\nЭтот блок выводит спецификации прямой\nv.0.1.0")
    while True:
        # ввод
        line1 = input("В уравнение вида Ax+By+C=0 описывающую прямую введите значения коэфициентов A, B, C подряд через пробелы\nB≠0\nНапример: 3 -4 -6\n")
        line1 = line1.split()
        # проверка 1
        if len(line1) != 3:
            print("Неверное количество символов!")
        ...
        if line1[0] == '0' or line1[0] == '-0':
            A = False
        else:
            A = True
        # --
        if line1[1] == '0' or line1[1] == '-0':
            B = False
        else:
            B = True
        # --
        if line1[2] == '0' or line1[2] == '-0':
            C = False
        else:
            C = True

        print(line_spec(A, B, C))
        return
def choice10():
    print("\nЭтот блок выводит уравнение прямой, проходящей через 2 точки\nv.0.1.0")
    while True:
        while True:
            # ввод1
            t1 = input("Введите координаты x и y ПЕРВОЙ точки подряд через пробелы\nНапример: 1 2\n")
            t1 = t1.split()
            # проверка 1
            if len(t1) != 2:
                print("Неверное количество символов!")
        ...
        while True:
            # ввод2
            t2 = input("Введите координаты x и y ВТОРОЙ точки подряд через пробелы\nНапример: 3 4\n")
            t2 = t2.split()
            # проверка 1
            if len(t2) != 2:
                print("Неверное количество символов!")
        ...
        for i in t:
            if int(i) < 0:
                ...
                out_t.append(i)
        print("(x-{})/({}-{}) = (y-{})/({}-{})\n".format(out_t[0], out_t[2], out_t[0], out_t[1], out_t[3], out_t[1]))
...
while run:
    ...
    print('Multi - Это сборник из 14 вычислительных программ и 7 скрытых вспомогательных утилит под авторством Тимофея\n')
    choice = input('''
...
8)Превести из Ax+By+C=0 или (x-x1)/(x2-x1)=(y-y1)/(y2-y1) (2 точки) в уравнение вида y = kx + b (***)
9)Вывести общие характеристики расположения прямой вида Ax+By+C=0 (вдруг она параллельна какой-нибудь оси и т.д.)
10)Уравнение прямой, проходящей  через 2 точки
11)Принадлежит ли точка прямой
(1-10) ''')
    ...
    # ----------------------------------------------------------------------------------------
    ex = input("Удивительный факт: в этой программе ровно 1273 строки!\n1)Продолжить\n2)Выход\n")
    if ex == "2":
        run = False
    elif ex == '1':
        pass
